...
def forward_by_encoders(inches, speed, stop_action):
    ...
    deg = (inches/(1.3*3.14159))*(2*3.14159)*(180/3.14159) #number of revolutions * 2pi rad/rev * 180 deg/rad
    left_motor.run_to_rel_pos(speed_sp=speed*8)
    right_motor.run_to_rel_pos(speed_sp=speed*8)
    left_motor.run_to_rel_pos(position_sp=deg)
    right_motor.run_to_rel_pos(position_sp=deg)
